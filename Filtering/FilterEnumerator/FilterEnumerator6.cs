...
namespace ForgeECS
{
  public struct FilterEnumerator<T1, T2, T3, T4, T5, T6>
    ...
    where T6 : struct, IValueComponent
  {
    ...
    public FilterEnumerator(World world, List<Archetype> archetypes, ComponentCache<T1> cache1, ComponentCache<T2> cache2, ComponentCache<T3> cache3, ComponentCache<T4> cache4, ComponentCache<T5> cache5, ComponentCache<T6> cache6)
    {
    ...
    public Iteration<T1, T2, T3, T4, T5, T6> Current => new(_enumeratorData.EntityId(), _world, _cache1,_cache2, _cache3, _cache4, _cache5, _cache6);
  }
}