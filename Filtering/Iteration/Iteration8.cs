namespace ForgeECS
{
  public readonly ref struct Iteration<T1, T2, T3, T4, T5, T6, T7, T8>
    ...
    where T8 : struct, IValueComponent
  {
    ...
    public Iteration(int entityId, World world, ComponentCache<T1> cache1, ComponentCache<T2> cache2, ComponentCache<T3> cache3, ComponentCache<T4> cache4, ComponentCache<T5> cache5, ComponentCache<T6> cache6, ComponentCache<T7> cache7, ComponentCache<T8> cache8)
    {
      _entityId = entityId;
      _world = world;
      _cache1 = cache1;
